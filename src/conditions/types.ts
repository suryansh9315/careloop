...
export function buildIntakeFlow(module: ConditionModule): FlowSpec {
  ...
  const itemNodes = items.map((item, i) => ({
    ...
    instruction: `Ask this ${module.instrument.name} question, then wait for their answer:\n"${item.prompt}"\nWhen you have a clear answer, you MUST record it by calling the chartLive tool — calling the tool is the ONLY thing that saves it: chartLive { linkId: "${item.linkId}", value: <${item.min}-${item.max} mapped as ${item.scale}>, note: "<their words>" }.\nHARD RULES: Do NOT repeat, paraphrase, or read their answer back. Do NOT say "I'll note that", "I'll record", "let me note", or describe what you're saving. Do NOT say the number. React with ONE brief, warm, EMPATHETIC acknowledgement that fits what they said (vary it — e.g. "that sounds tough", "glad to hear it", "thanks for sharing that") — never reuse the same phrase and never echo their words back — then ask the next question.`,
    edges: [
  ...
  const riskNodes = risks.map((rq, i) => ({
    ...
    type: 'subagent' as const,
    allowedTools: ['chartLive', 'getCareContext', 'checkCoverage'] as ToolName[],
    instruction: `Ask this follow-up question warmly, then wait:\n"${rq.prompt}"\nWhen you have their answer, you MUST call the chartLive tool — it is the only thing that saves it: chartLive { riskId: "${rq.id}", value: <integer per: ${rq.encoding}>, note: "<their words>" }.\nHARD RULES: Do NOT repeat or read their answer back, do NOT say "I'll note that" or mention recording/scoring. One short varied acknowledgement only, then move on.`,
    edges: [
      { to: 'answer_qa', when: 'the patient asks a question instead of answering' },
  ...
  return {
    ...
    nodes: [
      ...
      { id: 'submit', type: 'subagent', allowedTools: ['submitQuestionnaire'], instruction: `You MUST actually invoke the submitQuestionnaire tool now — do NOT just say you submitted; saying it without calling the tool loses all their answers. Call it SILENTLY (do not announce it or say "one moment"). The instant the tool returns, move straight into the value recap.`, edges: [{ to: 'recap', when: 'submitted' }] },
      { id: 'recap', type: 'subagent', allowedTools: ['getCareContext', 'checkCoverage', 'endCall'], instruction: `Give {{givenName}} a brief, warm PERSONALIZED wrap-up so the call feels worth their time:\n1) Reflect back the 1–2 most important things they shared today, in their own terms (e.g. more night-time symptoms, using the reliever a lot, a concern they raised).\n2) Offer ONE concrete, reassuring tip relevant to what they said — if useful, call getCareContext to ground it (e.g. inhaler technique, spacer, trigger avoidance). Keep it to one sentence, non-prescriptive.\n3) Tell them what happens next: their care team will review everything before the visit and follow up, and remind them to seek urgent care if breathing suddenly worsens.\n4) Then ASK if there's anything they'd like to ask before you finish, and WAIT. If they ask something, answer briefly — getCareContext for clinical/education questions, checkCoverage for insurance/cost — then check if there's anything else.\n5) ONLY once they confirm they have no more questions: say a warm goodbye, and THEN call the endCall tool to hang up (silently — do not mention it). Never call endCall before the goodbye or while they might still have something to say.\nWarm, unhurried, plain language. Do not read back numbers or scores.`, edges: [{ to: 'answer_qa', when: 'the patient asks a question' }, { to: 'close', when: 'goodbye said and endCall called' }] },
